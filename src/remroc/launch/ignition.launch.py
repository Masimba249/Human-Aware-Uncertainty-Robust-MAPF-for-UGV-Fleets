...
from launch.actions import IncludeLaunchDescription, SetEnvironmentVariable, DeclareLaunchArgument, RegisterEventHandler, LogInfo, GroupAction, TimerAction, EmitEvent, OpaqueFunction
...
# Modified 2026 (Collins Masimba): the coordinator, experiment yaml and a few human-aware
# coordination settings are launch arguments (so experiments can be batched, see
# scripts/run_remroc_experiments.sh), the mapf_solver_server is also started for
# coordinator_ha_cbs, and a human pose publisher and a coordinator-independent metrics
# recorder are launched; the simulation shuts down when the recorder finishes.
#
#   ros2 launch remroc ignition.launch.py coordinator:=coordinator_ha_cbs world:=narrow_corridors \
#       number_of_humans:=10 sample:=2

coordinator_default = 'coordinator_mapf' # The coordinator to use for the experiment. Must match the executable name of the ros2 node.
experiment_yaml_default = 'auto' # The experiment yaml ("auto": exp_ha_<world>.yaml if it exists, else exp_0_<world>.yaml). Located in the "params" folder.
...
MAPF_COORDINATORS = ['coordinator_mapf', 'coordinator_mapf_baseline', 'coordinator_ha_cbs']
...
ARGUMENTS = [
    ...
    DeclareLaunchArgument('coordinator', default_value=coordinator_default,
                          description='Coordinator executable (coordinator_mapf, coordinator_pbc, coordinator_ha_cbs, ...).'),
    DeclareLaunchArgument('experiment_yaml', default_value=experiment_yaml_default,
                          description='Experiment yaml in remroc/params ("auto" to derive it from the world).'),
    DeclareLaunchArgument('use_depot_mod', default_value='false', choices=['true', 'false'],
                          description='Use the modified depot map in which the "busy" area is blocked for the robots.'),
    DeclareLaunchArgument('results_dir', default_value='results', description='Where results are written.'),
    DeclareLaunchArgument('time_limit', default_value='600.0', description='Episode time limit [s].'),
    DeclareLaunchArgument('coordinator_delay', default_value='30.0',
                          description='Delay between the start of the navigation stacks and of the coordinator [s].'),
    DeclareLaunchArgument('risk_weight', default_value='4.0', description='coordinator_ha_cbs: lambda.'),
    DeclareLaunchArgument('prediction', default_value='cv', choices=['cv', 'mod'],
                          description='coordinator_ha_cbs: constant-velocity or map-of-dynamics prediction.'),
    DeclareLaunchArgument('enable_repair', default_value='true', choices=['true', 'false'],
                          description='coordinator_ha_cbs: local plan repair.'),
    DeclareLaunchArgument('human_source', default_value='script', choices=['script', 'gazebo'],
                          description='Human poses from the actor scripts of the world SDF or bridged from Gazebo.'),
]
...
def launch_setup(context, *args, **kwargs):
    ...
    coordinator = LaunchConfiguration('coordinator').perform(context)
    world_str = world_name.perform(context)
    use_depot_mod = LaunchConfiguration('use_depot_mod').perform(context) == 'true'
    if use_depot_mod:
    ...
    ld.add_action(
        ...
        condition=IfCondition(str(coordinator in MAPF_COORDINATORS).lower()),
        output='screen',
    ...
    experiment_yaml = LaunchConfiguration('experiment_yaml').perform(context)
    if experiment_yaml == 'auto':
        experiment_yaml = f'exp_ha_{world_str}.yaml'
        if not Path(pkg_remroc).joinpath('params', experiment_yaml).exists():
            experiment_yaml = f'exp_0_{world_str}.yaml'
    ...
    coordinator_parameters = [{'use_sim_time': LaunchConfiguration('use_sim_time')},
                              {'map_name': map_name,
                               'number_of_humans': number_of_humans,
                               'sample': sample,
                               'experiment_yaml': experiment_yaml}]
    if coordinator == 'coordinator_ha_cbs':
        coordinator_parameters.append({
            'risk_weight': float(LaunchConfiguration('risk_weight').perform(context)),
            'prediction': LaunchConfiguration('prediction'),
            'enable_repair': LaunchConfiguration('enable_repair').perform(context) == 'true',
            'time_limit': float(LaunchConfiguration('time_limit').perform(context)),
    ...
    # Human positions (for human-aware coordination and for the human-robot distance metrics).
    ld.add_action(
        Node(
            package='remroc',
            executable='human_pose_publisher',
            name='human_pose_publisher',
            output='screen',
            parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')},
                        {'world_name': world_name, 'number_of_humans': number_of_humans, 'sample': sample,
                         'source': LaunchConfiguration('human_source')}],
        )
    )
    if LaunchConfiguration('human_source').perform(context) == 'gazebo':
        ld.add_action(
            Node(
                package='ros_gz_bridge',
                executable='parameter_bridge',
                name='human_pose_bridge',
                output='screen',
                parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')}],
                arguments=[['/world/', world_name, '/dynamic_pose/info@tf2_msgs/msg/TFMessage[ignition.msgs.Pose_V']],
                remappings=[(['/world/', world_name, '/dynamic_pose/info'], '/humans/gz_poses')],
            )
        )

    # Coordinator-independent metrics (same definitions as the remroc_ha simulator).
    metrics_recorder_node = Node(
        ...
        output='screen',
        parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')},
                    {'coordinator': coordinator.replace('coordinator_', ''),
                     'map_name': map_name, 'number_of_humans': number_of_humans, 'sample': sample,
                     'experiment_yaml': experiment_yaml,
                     'results_dir': LaunchConfiguration('results_dir'),
                     'time_limit': float(LaunchConfiguration('time_limit').perform(context))}],
    )

    # This launches the respective coordinator node (and the recorder, so that both share the
    # same time origin) when all robots are launched. As it takes a while for all nav2
    # components to fully start up there is a delay, which depends on the number of robots
    # and the hardware on which the experiment are run.
    launch_experiment_node = RegisterEventHandler(
        OnExecutionComplete(
        target_action=group_action,
        on_completion=TimerAction(actions=[robot_experiment_node, metrics_recorder_node],
                                  period=float(LaunchConfiguration('coordinator_delay').perform(context)))
        )
    )
    ld.add_action(launch_experiment_node)

    # The recorder decides when the episode is over (all robots settled at their goals or
    # time limit): shut the simulation down so that experiments can be batched.
    my_shutdown_handler = RegisterEventHandler(
        OnProcessExit(
        target_action=metrics_recorder_node,
        on_exit=[
