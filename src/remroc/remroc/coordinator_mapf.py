...
class ExperimentAsync(Node):
    ...
    def __init__(self):
        ...
        if self.map_name in ['depot', 'depot_mod']:
            ...
            # Modified 2026: maps generated by remroc_world_generation (e.g. narrow_corridors)
            # store this grid metadata in their mapf yaml.
            with open(Path(self.pkg_remroc).joinpath('params', self.mapf_yaml), 'r') as mapf_file:
                grid_meta = yaml.safe_load(mapf_file).get('grid')
            if grid_meta is None:
                raise AssertionError('Unknown mapname! Could not launch experiment node.')
            map_resolution = grid_meta['resolution']
            grid_cell_size = grid_meta['cell_size_px']
            x_offset = grid_meta['x_offset']
            y_offset = grid_meta['y_offset']
            map_origin = grid_meta['origin']
